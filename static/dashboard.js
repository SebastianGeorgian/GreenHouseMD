/* ──────────────────────────────────────────
   Greenhouse Dashboard JS — no emoji, SVG icons
────────────────────────────────────────── */
...
function renderMiniStats(tempData, humData) {
  ...
  let html = '';
  if (t) {
    html += `<div class="stat-chip temp-chip">min temp <strong>${t.min}\u00b0</strong></div>`;
    html += `<div class="stat-chip temp-chip">avg <strong>${t.avg}\u00b0</strong></div>`;
    html += `<div class="stat-chip temp-chip">max <strong>${t.max}\u00b0</strong></div>`;
  }
  if (h) {
    html += `<div class="stat-chip hum-chip">min hum <strong>${h.min}%</strong></div>`;
    html += `<div class="stat-chip hum-chip">avg <strong>${h.avg}%</strong></div>`;
...
function buildLegend(metric) {
  return metric === 'humidity_avg_6min'
    ? 'humidity: &lt;35% · 35–55% · 55–75% · &gt;75%'
    : 'temperature: &lt;15° · 15–20° · 20–25° · 25–30° · &gt;30°';
}
...
function renderHeatmapDayGrid(payload) {
  ...
  payload.values.forEach((v, i) => {
    ...
    cell.title = `${payload.date} ${payload.labels[i]} → ${v ?? 'no data'}`;
    cell.innerHTML = `
...
function buildEventTooltip(label) {
  ...
  return {
    ...
    cornerRadius: 8,
    callbacks: {
      label: ctx => `${label}: ${ctx.parsed.y === 1 ? 'Detected' : 'None'}`,
    },
  };
}
...
// Time window for the current selection
...
function renderEventSummary(containerId, data, sensorLabel, colorClass) {
  ...
  if (totalPoints === 0) {
    el.innerHTML = `<div class="ev-chip">No data for the selected period</div>`;
    return;
  }

  el.innerHTML = `
    <div class="ev-chip ${colorClass}">
      detections <strong>${totalEvents}</strong>
    </div>
    <div class="ev-chip ${colorClass}">
      active time <strong>${pct}%</strong>
    </div>
    <div class="ev-chip">
      samples <strong>${totalPoints}</strong>
    </div>
  `;
}
...
async function loadEventChart(chart, sensor, dateInputId, rangeSelectId, summaryId, colorClass, sensorLabel) {
  ...
  try {
    ...
    if (el) el.innerHTML = `<div class="ev-chip">Error loading data</div>`;
  }
}

// ── Status card auto-refresh ─────────────────
const ALERT_SENSORS = ['rain', 'fire', 'gas', 'soil'];
const CARD_LABELS   = { rain: 'Rain', fire: 'Flame', gas: 'Gas', soil: 'Soil moisture' };
const CARD_ICONS    = {
  rain: '#i-rain',
  fire: '#i-fire',
  gas:  '#i-gas',
...
// Human-readable text instead of 0/1
const CARD_STATES = {
  rain: { on: 'Detected', off: 'None' },
  fire: { on: 'Detected', off: 'None' },
  gas:  { on: 'Detected', off: 'None'  },
  soil: { on: 'Wet',      off: 'Dry'   },
};
...
function updateLiveBadge(d) {
  ...
  if (d.climate_offline) {
    ...
    if (d.climate_age_sec === null) {
      text.textContent = 'no data from sensors';
    } else {
      const min = Math.round(d.climate_age_sec / 60);
      text.textContent = `climate sensor offline · ${min} min`;
    }
  } else {
...
async function refreshStatus() {
  try {
    ...
    // online/offline badge + indoor values always update
    updateLiveBadge(d);
    _inside.temp = d.temp;
    _inside.hum  = d.hum;
    renderWeatherStrip();

    // don't rebuild the cards if nothing changed
...
function renderWeatherStrip() {
  ...
  el.innerHTML = `
    <div class="ws-group">
      ...
      <span class="ws-label">Outside</span>
    ...
    <div class="ws-group">
      ...
      <span class="ws-label">Greenhouse</span>
...
// ── Alert history ─────────────────────────────
const ALERT_NAMES = { fire: 'Flame', gas: 'Gas' };
...
async function loadAlerts() {
  ...
  try {
    ...
    if (!d.alerts.length) {
      el.innerHTML = `<div class="alerts-empty">No alerts recorded — the greenhouse is safe.</div>`;
      return;
    }
    ...
    el.innerHTML = d.alerts.map(a => `
      <div class="alert-row ${a.ongoing ? 'ongoing' : ''}">
        ...
        <span class="alert-duration">${a.ongoing ? 'ongoing' : formatDuration(a.duration_sec)}</span>
      </div>
    `).join('');
  } catch (e) {
    console.warn('Alerts load failed:', e);
    el.innerHTML = `<div class="alerts-empty">Error loading alert history.</div>`;
  }
}
...
window.onload = () => {
  ...
  // Outside weather + alert history
  loadWeather();
  setInterval(loadWeather, 10 * 60 * 1000);   // every 10 minutes
  loadAlerts();
  setInterval(loadAlerts, 60 * 1000);         // every 1 minute
  ...
  // Event charts
  const C = getColors();
  rainChart = initEventChart('rainChart', C.rain, C.rainFill, 'Rain');
  soilChart = initEventChart('soilChart', C.soil, C.soilFill, 'Soil moisture');

  document.getElementById('rainDate').value = todayStr;
  document.getElementById('soilDate').value = todayStr;

  loadEventChart(rainChart, 'rain', 'rainDate', 'rainRange', 'rainSummary', 'rain-chip', 'Rain');
  loadEventChart(soilChart, 'soil', 'soilDate', 'soilRange', 'soilSummary', 'soil-chip', 'Soil moisture');
